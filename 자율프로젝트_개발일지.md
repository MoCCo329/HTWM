...
electron과 react간 ipc 통신을 확인했다. 모션인식에 사용되는 라즈베리파이에서 왭과 음성인식을 처리하는 라즈베리 파이로 데이터를 전송하면 electron이 react로 다시 ipc 통신을 하고, react에서 해당 데이터를 보여주거나 서버로 보내는 등의 데이터 처리를 진행한다.
